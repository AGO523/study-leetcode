...
# ノードとは？
# ノード（Node）は、データ構造における基本的な単位です。リンクリストのノードは、次の要素を持ちます：

# 値（Value）:
# ノードが保持するデータ。
# 次のノードへの参照（Next Pointer）:
# リンクリスト内の次のノードを指す参照（ポインタ）。
# リンクリストの基本構造
# リンクリストは、ノードがチェーン状に連結されたデータ構造です。各ノードは次のノードへの参照を持ち、最後のノードは nil を指します。
